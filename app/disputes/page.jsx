...
import { AlertCircle, Calendar, Eye, Search, RefreshCw, ChevronRight } from 'lucide-react';
...
export default function ClientDisputesPage() {
  ...
  const filteredDisputes = disputes.filter((d) => {
    ...
    return (
      ...
      d.freelancer?.name?.toLowerCase().includes(q)
    );
  });

  return (
    <div className="animate-in fade-in duration-300 pt-6 pb-10 px-4">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-foreground">Dispute Orders</h1>
            <p className="text-sm text-muted-foreground mt-1">
              Tickets for your orders. Open a dispute from the order page; view and reply here.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredDisputes.map((dispute) => (
              <Link key={dispute.id} href={`/orders/${dispute.orderId}`} className="group block h-full">
                <Card className="h-full overflow-hidden border-border hover:border-primary/40 hover:shadow-lg transition-all duration-200">
                  <CardContent className="p-4 flex flex-col h-full">
                    ...
                    <div className="text-xs text-muted-foreground mt-2 space-y-0.5">
                      ...
                      <p>Seller: {dispute.freelancer?.name || '—'}</p>
                    </div>

...
export async function createOrder({
  ...
  paymentMethodUsed, // Payment method selected by client (must be one of service's accepted methods)
  clientIpAddress,
}) {
  // Get service details
 ...
 * When service has payment methods, paymentMethodUsed is required.
 */
export async function acceptDelivery(orderId, clientId, deliverableId, paymentMethodUsed = null) {
  const order = await prisma.order.findUnique({
    where: { id: orderId },
    include: { deliverables: true, service: true },
  });
  ...
  const servicePaymentMethods = order.service?.paymentMethods && Array.isArray(order.service.paymentMethods)
    ...
    : [];
  if (servicePaymentMethods.length > 0 && !paymentMethodUsed) {
    throw new Error('Please select a payment method to complete this order.');
  }
  if (servicePaymentMethods.length > 0 && paymentMethodUsed && !servicePaymentMethods.includes(paymentMethodUsed)) {
    throw new Error('Selected payment method is not accepted for this service.');
  }
  ...
  // Update order to COMPLETED (and save payment method if provided)
  const updatedOrder = await prisma.order.update({
    where: { id: orderId },
    data: {
      status: 'COMPLETED',
...
/**
 * Get user's orders
 * - CLIENT: orders where clientId = userId
 * - FREELANCER: orders where freelancerId = userId
 * - CLIENT + isApprovedSeller: orders where clientId OR freelancerId = userId (both roles)
 * - ADMIN: all orders
 */
export async function getUserOrders(userId, userRole, options = {}) {
  const { status, serviceId, limit = 50, skip = 0, isApprovedSeller = false } = options;

  let participantFilter = {};
  if (userRole === 'ADMIN') {
    participantFilter = {}; // Admin sees all
  } else if (userRole === 'FREELANCER') {
    participantFilter = { freelancerId: userId };
  } else if (userRole === 'CLIENT' && isApprovedSeller) {
    // Client who became approved seller: see both client and freelancer orders
    participantFilter = {
      OR: [{ clientId: userId }, { freelancerId: userId }],
    };
  } else if (userRole === 'CLIENT') {
    participantFilter = { clientId: userId };
  }

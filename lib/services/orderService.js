...
export async function createOrder({
  ...
  // Check if there's already a pending/active order for this client-freelancer pair
  // (Only if freelancer is creating - prevent multiple pending contracts)
  if (isFreelancerCreating) {
    // First check for pending contracts (can be replaced)
    const whereClausePending = {
      clientId: actualClientId,
      freelancerId: actualFreelancerId,
      status: 'PENDING_ACCEPTANCE',
    };

    // If conversationId exists, prioritize checking that conversation
    if (conversationId) {
      whereClausePending.conversationId = conversationId;
    }
    ...
    // Check for active orders (cannot be replaced)
    const whereClauseActive = {
      clientId: actualClientId,
      freelancerId: actualFreelancerId,
      status: {
        in: ['IN_PROGRESS', 'DELIVERED', 'REVISION_REQUESTED'],
      },
    };

    if (conversationId) {
      whereClauseActive.conversationId = conversationId;
    }
    ...
    // If there's an active order, block creation
    if (existingActiveOrder) {
      throw new Error('An active order already exists with this client. Please complete or cancel the existing order first.');
    }

    // If there's a pending order, cancel it and allow new one
        ...
        data: {
          ...
          contract: {
            update: {
              status: 'REJECTED',
              rejectedAt: new Date(),
              rejectedBy: actualFreelancerId,
              rejectionReason: 'Replaced by new contract',
            },
          },
          events: {
            create: {
              ...
              description: 'Previous contract cancelled - new contract created',
              metadata: {
                reason: 'Replaced by new contract',
                replacedBy: 'FREELANCER',
              },
            },
          },
        },

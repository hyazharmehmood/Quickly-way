...
export async function rejectOrder(orderId, userId, userRole, rejectionReason) {
  ...
  const updatedOrder = await prisma.order.update({
    ...
    data: {
      ...
      cancelledBy: userId, // Client who rejected the contract
      cancellationReason: rejectionReason,
      contract: {
        update: {
          status: 'REJECTED',
          rejectedAt: new Date(),
          rejectedBy: userId, // Client who rejected
          rejectionReason,
        },
      },
      events: {
        create: {
          userId: userId, // Client who rejected
  ...
  await prisma.orderEvent.create({
    data: {
      orderId,
      userId: order.freelancerId, // Freelancer who created the contract

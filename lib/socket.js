  ...
  io.on('connection', (socket) => {
    ...
    socket.on('fetch_messages', async (data) => {
      try {
        ...
        console.log('📋 Fetching offers for messages:', {
          offerIds,
          offerMessagesCount: offerIds.length,
        });

        // Note: Orders are now created from offers
        // Orders will be included in offer.order when offer is accepted
        ...
        // Attach offer data to messages (just like sender is included automatically)
        // Note: Orders are created from offers
        const messagesWithData = messages.map((message) => {
          ...
          if (message.type === 'offer' && message.attachmentName) {
            ...
            return {
              ...
              order: offer?.order || null, // Include order if offer was accepted
            };
          }

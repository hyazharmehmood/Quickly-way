...
const initSocket = async (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Fetch messages for a conversation - OPTIMIZED with Redis caching and parallel queries
      ...
      try {
        ...
        if (page === 1) {
          ...
          if (cachedMessages && cachedMessages.length > 0) {
            ...
            (async () => {
              try {
                await fetchAndCacheMessages(conversationId, userId, limit);
              } catch (err) {
        ...
        // Verify user is a participant and fetch messages in parallel
        ...
        // Cache results for page 1 (last 20 messages)
        if (page === 1) {
    ...
    socket.on('send_message', async (data) => {
      try {
        ...
        // Clear message cache for this conversation and add new message
        await clearCachedMessages(conversationId);
        // Add new message to cache immediately (optimistic update)
        const cachedMessages = await getCachedMessages(conversationId) || [];
        cachedMessages.push(messageWithData);
        await setCachedMessages(conversationId, cachedMessages.slice(-CACHE_LIMIT));

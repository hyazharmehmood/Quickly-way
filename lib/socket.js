  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join_conversation', async (conversationId) => {
      try {
        ...
        if (updatedConversation) {
          ...
          // Always emit conversation:updated to user's personal room with unreadCount: 0 (from ConversationParticipant)
          io.to(`user:${userId}`).emit('conversation:updated', {
            conversation: {
              ...updatedConversation,
              otherParticipant: updatedConversation.participants.find((p) => p.userId !== userId)?.user,
              unreadCount: 0, // ALWAYS from ConversationParticipant.unreadCount (0 when conversation is open)
              // Format lastMessage with status tracking
              lastMessage: actualLastMessage || (updatedConversation.lastMessageText ? {
                id: 'denormalized',
                content: updatedConversation.lastMessageText,
    ...
    socket.on('get_conversations', async () => {
      try {
        ...
        const participantRecords = await prisma.conversationParticipant.findMany({
          ...
          orderBy: {
            conversation: {
              lastMessageAt: 'desc', // Use lastMessageAt for conversation ordering (WhatsApp/Fiverr style)
            },
          },
        });
    ...
    socket.on('send_message', async (data) => {
      try {
        ...
        // 5️⃣ Update conversation denormalized fields - use lastMessageAt for ordering
        await prisma.conversation.update({
          where: { id: conversationId },
          data: {
            lastMessageText: content.trim(),
            lastMessageAt: now, // CRITICAL: Use this for conversation ordering (not updatedAt)
            lastSenderId: userId,
            updatedAt: now,
          },
        });

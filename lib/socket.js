...
const initSocket = (server) => {
  ...
  io.on('connection', (socket) => {
    ...
    // Dispute thread real-time: join room when viewing dispute (like chat)
    socket.on('dispute:subscribe', (data) => {
      const disputeId = data?.disputeId != null ? String(data.disputeId) : null;
      if (disputeId) {
    ...
    // Support ticket messages real-time (client + admin viewing ticket)
    socket.on('supportTicket:subscribe', (data) => {
      const ticketId = data?.ticketId != null ? String(data.ticketId) : null;
      if (ticketId) {
        socket.join(`supportTicket:${ticketId}`);
      }
    });

    socket.on('supportTicket:unsubscribe', (data) => {
      const ticketId = data?.ticketId != null ? String(data.ticketId) : null;
      if (ticketId) {
        socket.leave(`supportTicket:${ticketId}`);
      }
    });
        ...
        for (const p of updatedParticipants) {
          ...
          if (p.userId !== userId && (participantData?.unreadCount || 0) > 0) {
            ...
            try {
              ...
              io.to(`user:${p.userId}`).emit('notification:new', payload);
            } catch (err) {
...
const emitDisputeCommentEvent = (disputeId, comment, { clientId, freelancerId } = {}) => {
  ...
  ioInstance.to(`dispute:${roomId}`).emit('dispute:new_comment', eventData);
  console.log(`📢 Emitted dispute:new_comment to dispute:${roomId}`);
...
/**
 * Emit support ticket message event - real-time update when someone adds a message
 * Emits to supportTicket room (client or admin viewing the ticket)
 */
const emitSupportTicketMessageEvent = (ticketId, message, { createdById, assignedAgentId } = {}) => {
  const ioInstance = io || globalThis?.__SOCKET_IO__ || global?.__SOCKET_IO__;
  if (!ioInstance) {
    console.warn('Socket.IO not initialized, cannot emit support ticket message event');
    return;
  }
  ...
  ioInstance.to(`supportTicket:${roomId}`).emit('supportTicket:new_message', eventData);
  console.log(`📢 Emitted supportTicket:new_message to supportTicket:${roomId}`);

  if (createdById) {
    ioInstance.to(`user:${createdById}`).emit('supportTicket:new_message', eventData);
  }
  if (assignedAgentId && assignedAgentId !== createdById) {
    ioInstance.to(`user:${assignedAgentId}`).emit('supportTicket:new_message', eventData);
  }
};

...
const initSocket = (server) => {
  ...
  // Helper to get online freelancer IDs: FREELANCER role OR approved seller (CLIENT + isSeller)
  const getOnlineFreelancerIds = () => {
    return Array.from(onlineUsers.entries())
      .filter(([userId, data]) => {
        if (data.role === 'FREELANCER') return true;
        if (data.role === 'CLIENT' && data.isSeller) return true; // Approved seller (client who became freelancer)
        return false;
      })
      .map(([userId]) => userId);
  };
  ...
  publicNamespace.on('connection', (socket) => {
    const onlineIds = getOnlineFreelancerIds();
              ...
              conversationData.participants
                ...
                .forEach((otherUserId) => {
                  if (onlineUsers.has(otherUserId)) {
                    io.to(onlineUsers.get(otherUserId).socketId).emit('messages_read', {
                      conversationId,
                      readerId: userId,
                    });
                  }

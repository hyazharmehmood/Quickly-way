...
export async function getServiceById(serviceId) {
    ...
    try {
        // 1. Get service-based reviews (direct gig reviews)
        const serviceReviews = await prisma.review.findMany({
            where: {
                serviceId,
            ...
            orderBy: { createdAt: 'desc' },
        }).catch(() => []); // Return empty array if Review table doesn't exist
        ...
        if (completedOrders.length > 0) {
            const orderIds = completedOrders.map(o => o.id);
            const orderReviewsData = await prisma.review.findMany({
                where: {
                    orderId: { in: orderIds },
                    isOrderReview: true,
